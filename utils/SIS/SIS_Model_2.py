...
def Model_2(G, H, a, b, beta, mu, tmin, tmax):
    ...
    while infecteds and t<tmax:
        events = ['recovery', 'infection', 'nodeoff', 'nodeon'] #use random.choices() to choose the next event based on its probability
        ...
        total_transmission_rate = beta*IS_links.total_weight()
        total_node_on_rate = a*inactivateds.total_weight()
        total_node_off_rate = b*activateds.total_weight()      #I fixed one error here (same for models 1,3,4). I wrote down the word 'ndoe' instead of 'node' before, which is so stupid.
        total_rate = total_recovery_rate + total_transmission_rate + total_node_on_rate + total_node_off_rate
        if total_rate > 0:
            delay = random.expovariate(total_rate)
        else:
