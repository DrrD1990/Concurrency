...
def Model_2(G, H, a, b, beta, mu, initial_infecteds, initial_recovereds, tmin = 0, tmax = float('Inf')):
    r'''    
    ...
    **initial_recovereds**
    ...
    for node in G.nodes():#randomly chooses nodes in G according to the given probability and then add it to the set of high active nodes
        if random.random() < (7-2*math.sqrt(10))/(7*math.sqrt(10)-20):
            activeness[node] = 'A' #'A'for high activity state
            for nbr in H.neighbors(node):
                if activeness[nbr]=='A':
                    G.add_edge(node,nbr)#add edges being active due to the change of states of nodes
    ...
    for node in G.nodes():
        if activeness[node]=='N':
            inactivateds.update(node)#add the initial low active nodes
        else:
            activateds.update(node)#add the initial high active nodes
    ...
    while infecteds and t<tmax:
        events = ['recovery', 'infection', 'nodeoff', 'nodeon'] #choose the next event based on its probability
        ...
        if rand == ['recovery']:
            ...
            for nbr in H.neighbors(activating_node):
                if nbr != activating_node and activeness[nbr] == 'A':
...
a = 7-2*math.sqrt(10) #rate from node being low active to high active
b = 9*math.sqrt(10)-27 #rate from node being high active to low active 
beta_min = 0.0
beta_max = 10.01
beta_gap = 0.25
mu = 1.0
...
for beta in np.arange(beta_min, beta_max, beta_gap):
    ...
    for i in range(iteration):
        ...
        f = Model_2(G, H, a, b, beta, mu, initial_infecteds, initial_recovereds, tmin = 0, tmax = float('Inf'))
        fraction = f / n
        fraction_list.append(fraction)
    fraction_mean = np.mean(fraction_list)
    fraction_of_recovered.append(fraction_mean)

np.savetxt('M2_q0.1_f_1k5.txt', fraction_of_recovered)
